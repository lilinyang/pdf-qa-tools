...
with st.sidebar:
    openai_api_key = st.text_input(
        "请输入百炼（DashScope）API 密钥：",
        type="password",
        help="请在阿里云百炼控制台获取 API Key",
    )
    st.markdown(
        "[获取百炼 API Key]"
...
if uploaded_file:
    ...
    if uploaded_file.size > MAX_FILE_SIZE_MB * 1024 * 1024:
        st.error(
            f"文件大小 ({uploaded_file.size / 1024 / 1024:.1f}MB) "
            f"超过限制 ({MAX_FILE_SIZE_MB}MB)"
        )
        uploaded_file = None
    # 空文件校验
    elif uploaded_file.size == 0:
        st.error("上传的文件为空（0 字节），请重新选择一个有效的 PDF 文件。")
        uploaded_file = None
    else:
        st.info(f"文件: {uploaded_file.name} ({uploaded_file.size / 1024:.1f} KB)")
...
if uploaded_file and openai_api_key:
    ...
    if st.session_state["file_id"] != file_id:
        with st.spinner("正在解析PDF并构建向量库（首次需要等待）..."):
            start = time.time()
            try:
                retriever, num_chunks = process_pdf_to_retriever(
                    uploaded_file, openai_api_key
                )
                elapsed = time.time() - start
                st.session_state["retriever"] = retriever
                st.session_state["file_id"] = file_id
                st.session_state["chat_history"] = []  # 换文件时清空历史
                st.success(
                    f"文档处理完成！{num_chunks} 个片段，耗时 {elapsed:.1f}秒"
                )
            except Exception as e:
                st.error(f"文档处理失败：{e}")
                # 处理失败时不更新 session_state，避免后续流程出错
    else:
        st.success("向量库已就绪，直接提问即可")

elif uploaded_file and not openai_api_key:
    st.warning("请先在左侧输入百炼 API 密钥")
...
if question and st.session_state["retriever"] and openai_api_key:
    ...
    st.warning("请先输入百炼 API 密钥")

# --- 历史消息 ---
if st.session_state["chat_history"]:
    with st.expander(
        f"历史消息（最近 {len(st.session_state['chat_history']) // 2} 轮）"
    ):
        for i in range(0, len(st.session_state["chat_history"]), 2):
            human_msg = st.session_state["chat_history"][i]
            ai_msg = st.session_state["chat_history"][i + 1]
